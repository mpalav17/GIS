...
def getn_by_N(cc):    
    df = pd.read_csv('C:/Users/Rishabh/waterbudgeting/Data/n_by_N(oktas).csv')
    print(math.ceil(cc*5))
    return df['n_by_N'][math.ceil(cc*5)]
def get_ea(temp_mean):
    df = pd.read_csv('C:/Users/Rishabh/waterbudgeting/Data/SaturationVapourPressure.csv')
...
def getLatitude(village):
    ...
    if(village == 'Konambe' or village=='Shastrinagar' or village=='Kirtangali'):
        return 19.84
    elif(village == 'Kanhur'):
        return 18.8250
def getElevation(village):
    #elvation of Konambe village in m
    if(village == 'Konambe' or village=='Kirtangali'):
        return 561
